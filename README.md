...
<title>Happy Birthday Lipsita — Together Forever</title>
<style>
...
.page{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;padding:2rem;transition:transform 700ms cubic-bezier(.2,.9,.2,1),opacity 500ms;backface-visibility:hidden}
.page.hidden{opacity:0;transform:translateY(20px) scale(.98);pointer-events:none}
.card{max-width:900px;width:100%;background:linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0.02));border:1px solid rgba(255,255,255,0.06);border-radius:18px;padding:3rem;text-align:center;box-shadow:0 10px 30px rgba(2,6,23,0.6)}
...
@media (max-width:600px){.shayari{font-size:1.4rem}.card{padding:2rem;border-radius:14px}}
...
.gallery-grid img{width:100%;border-radius:14px;object-fit:cover;max-height:260px;box-shadow:0 10px 30px rgba(15,23,42,0.8)}
...
<aside class="side-slider" aria-label="Scroll slider">
    <span class="small">Scroll</span>
    <input id="scrollSlider" type="range" orient="vertical" min="0" max="100" value="0" step="1" aria-label="Slide the whole page">
...
<div class="shayari">1) "Tere hatho pe muskan, tere dil me pyaar —<br>har pal mera tera, har lamha tera yaar."</div>
<div class="controls"><button class="btn" id="next1">Next →</button></div>
</div>
</section>
...
<h1>Shayari — Page 2</h1>
<h2 class="small">Romantic sairi</h2>
<div class="shayari">2) "Zindagi ki raato me, tera saath ho saath —<br>khushiyon ka ho samaan, mile sadaa har raat."</div>
<div class="controls"><button class="btn" id="prev2">← Prev</button><button class="btn" id="next2">Next →</button></div>
</div>
</section>
...
<h1>Shayari — Page 3</h1>
<h2 class="small">Short and sweet</h2>
<div class="shayari">3) "Har subah tera noor, har shaam tera pyaar —<br>sath chale hum sada, bharen zindagi pyaar se yaar."</div>
<div class="controls"><button class="btn" id="prev3">← Prev</button><button class="btn" id="next3">Next →</button></div>
</div>
</section>
...
<h1 class="glitter">Happy Birthday, Meri Little Mommiii💖!</h1>
<h2 class="small">May your day be as bright and lovely as you are.</h2>
<div style="margin-top:12px;font-weight:700;font-size:1.2rem;color:#fff">Together forever ❤️</div>
<div style="margin-top:10px;color:rgba(255,255,255,0.8)">— With all my love</div>
</div>
...
<div class="controls">
<button class="btn" id="prev4">← Prev</button>
<button class="btn" id="replay">Replay</button>
<button class="btn" id="cutCake">Cut Cake 🎂</button>
</div>
...
<div id="galleryOverlay" class="gallery-overlay" aria-hidden="true">
    <div class="gallery-inner">
        <div class="gallery-header">
            <div>
                <div class="gallery-title">My favourite photos of you 💖</div>
        ...
        <div class="gallery-grid">
            ...
            <img src="WhatsApp Image 2025-12-04 at 02.42.45.jpeg" alt="photo 1">
...
<div class="hint">Use ← and → or click Next/Prev — tap cards on mobile</div>
...
unlockGift.addEventListener('click',()=>{
    if(giftPassword.value==="MeriMommi"){
        if(galleryOverlay){
...
async function downloadAllImages() {
    ...
    const fetchPromises = imgElements.map(async (img, index) => {
        const url = img.src;
        try {
            ...
            const fileName = fileNameFromUrl || `image-${index + 1}.jpg`;
